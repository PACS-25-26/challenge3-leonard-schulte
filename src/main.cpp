...
int main(int argc, char* argv[]) {
    ...
    std::vector<double> uh(N*N, 0.0);        // u(x,y) stored linear with n*n Nodes
    std::vector<double> uh_new(N*N, 0.0);    // Current iteration solution (temporary)
    ...
    while (error > tol && p < maxIter)
    {
        for (int i = 1; i < N-1; ++i) {
            ...
            for (int j = 1; j < N-1; ++j) {
                ...
                uh_new[k] = 0.25 * (uh[k-N] + uh[k+N] + uh[k-1] + uh[k+1] + h*h * source[k]);
            }
        }
        error = l2_norm(uh, uh_new, h);
        l2    = l2_norm(u_exact, uh_new, h);
    ...
    std::cout << '\n';
    vtk_output(N, h, uh, x_min, y_min, case_name);
